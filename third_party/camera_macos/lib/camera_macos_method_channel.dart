...
class MethodChannelCameraMacOS extends CameraMacOSPlatform {
  ...
  Future<CameraMacOSArguments?> initialize(
      ...
      bool isVideoMirrored = false}) async {
    try {
  ...
  Future<void> _genericMethodCallHandler(MethodCall call) async {
    switch (call.method) {
      case "onVideoRecordingFinished":
        ...
        break;
      case "onPoseResult":
        if (registeredCallbacks["onPoseResult"] != null) {
          dynamic args = call.arguments;
          ...
          if (args is Map) {
            if (args["error"] != null) {
              ...
              result = CameraPoseResult.fromMap(args);
            }
          }
          registeredCallbacks["onPoseResult"]!(result, exception);
        }
        break;
      default:
        break;
  ...
  void setPoseResultListener(
    Function(CameraPoseResult?, CameraMacOSException?)? onPoseResult,
  ) {
    if (!methodCallHandlerSet) {
      methodChannel.setMethodCallHandler(_genericMethodCallHandler);
      methodCallHandlerSet = true;
    }
    registeredCallbacks["onPoseResult"] = onPoseResult;
  }
